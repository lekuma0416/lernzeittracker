...
import { LogOut, Timer, BookOpen, BarChart2, Calendar } from 'lucide-react'
...
export default function Navbar({ activeTab, setActiveTab }) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-gray-900 border-t border-gray-800 px-4 pb-safe z-50 md:static md:border-t-0 md:border-b md:border-gray-800">
      <div className="max-w-2xl mx-auto flex items-center justify-between py-2">
        <span className="hidden md:block text-white font-bold text-lg">Lernzeit</span>
          ...
          {TABS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex flex-col md:flex-row items-center gap-1 md:gap-1.5 px-3 py-2 rounded-xl text-xs md:text-sm font-medium transition-colors ${
                activeTab === id ? 'text-indigo-400 bg-indigo-500/10' : 'text-gray-500 hover:text-gray-300'
              }`}
            >
