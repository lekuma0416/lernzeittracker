        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
          ...
          return (
            <div
              ...
              style={{ backgroundColor: data ? `rgba(99,102,241,${intensity * 0.6})` : '#1f2937' }}
              title={data ? `${formatDurationHuman(data.seconds)}` : ''}
            >
              <span className={`font-medium ${isToday ? 'text-indigo-300' : data ? 'text-white' : 'text-gray-600'}`}>{day}</span>
              {subjColors.length > 0 && (
