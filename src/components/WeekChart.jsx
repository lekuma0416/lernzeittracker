...
export default function WeekChart({ sessions }) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm shadow">
        ...
        <p className="text-gray-900 dark:text-white font-semibold">{h > 0 ? `${h}h ${m}min` : `${m}min`}</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-200 dark:border-gray-800">
      <h2 className="text-gray-500 dark:text-gray-400 text-sm font-medium uppercase tracking-wider mb-4">Letzte 7 Tage</h2>
      <ResponsiveContainer width="100%" height={160}>
        <BarChart data={data} barSize={28}>
          ...
          <Bar dataKey="minutes" radius={[6, 6, 0, 0]}>
            {data.map((entry, i) => (
              <Cell key={i} fill={entry.isToday ? '#6366f1' : '#e5e7eb'} className="dark:[fill:#374151]" />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
